...
#  [cluster] host[1-2] and ip[1-2] can be changed by cluster.
#     Cluster applies a template that uses facts and exported resources to configure the resource.
#     WARN: One reppupet will be needed
...
define drbd::resource (
  ...
  concat { "/etc/drbd.d/${name}.res":
    ...
    before => Drbd::Resource::Enable[$name]
  }
  ...
  concat::fragment { "${name} drbd header":
    ...
    content => template("${module_name}/header.res.erb"),
    order   => '01',
  }
  ...
  if $ha_primary and $cluster {
    ...
    @@concat::fragment { "${name} ${cluster} primary resource":
      target  => "/etc/drbd.d/${name}.res",
      content => template("${module_name}/resource.res.erb"),
      order   => '10',
    }
    ...
    concat::fragment { "${name} static primary resource":
      target  => "/etc/drbd.d/${name}.res",
      content => template("${module_name}/primary-resource.res.erb"),
      order   => '10',
    }

    concat::fragment { "${name} static secondary resource":
      target  => "/etc/drbd.d/${name}.res",
      content => template("${module_name}/secondary-resource.res.erb"),
      order   => '20',
    }
